INSERT INTO public.lesson (name, content) VALUES ('First source code', e'When you want to write your program in C you have to write so called "main" function.
Functions in C/C++ are a pieces of code that have own name (indentificator) and body. Main function represents whole program, so it must be always written in source code.
...
');
INSERT INTO public.lesson (name, content) VALUES ('Header files', e'A header file is a file with extension .h which contains declarations and definitions to be shared between several source files. There are two types of header files: the files that the programmer writes and the files that comes with your compiler.
...
');
INSERT INTO public.lesson (name, content) VALUES ('Common simple input/output', e'Almost every program needs to input some values and output results. So we have to use appropriate commands for it.
...
');
INSERT INTO public.lesson (name, content) VALUES ('What is a variable?', e'A variable is certain place in computer memory which can hold some values. It''s different approach than in mathematics.
...
');
INSERT INTO public.lesson (name, content) VALUES ('Values of variable', e'Each variable can hold some values. Because the variable occupies certain space in the computer''s memory, which is composed as a sequence of bits, each value is machine-displayed as a sequence of zeros and ones.
...
');
-- INSERT INTO public.lesson (name, content) VALUES ('name', 'content');